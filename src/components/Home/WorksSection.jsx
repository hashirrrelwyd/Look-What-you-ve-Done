...
// Set constants for better maintainability
const CARD_SPACING_X = 200; // Horizontal spacing
const CARD_STAGGER_Y = 40; // Vertical stagger
const CARD_SCALE_DECREMENT = 0.15; // Scale reduction per step
...
export default function WorksSection() {
  ...
  return (
    <div className="lg:min-h-screen flex px-6 md:px-10 py-10 bg-[#111111] rounded-b-4xl text-white overflow-x-hidden">
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 w-full">
        {/* Left content - Desktop (Unchanged) */}
        <div className="hidden xl:flex flex-col justify-between items-start w-1/3">
          <h2
        ...
        {/* Carousel Container */}
        <div className="relative w-full lg:w-2/3 flex items-center justify-center lg:pl-6">
          {/* Desktop Carousel (Optimized for smooth rotation) */}
          <div className="hidden xl:block relative w-full h-[550px]"> {/* Increased height for full visibility */}
            <AnimatePresence initial={false}>
              {visibleCards.map((card, index) => {
                const isActive = index === 0;

                // Calculate positioning for staggered effect based on index in visibleCards
                const leftOffset = index * CARD_SPACING_X;
                const topOffset = index * CARD_STAGGER_Y;
                const scale = 1 - index * CARD_SCALE_DECREMENT;
                const zIndex = 4 - index;
                ...
                return (
                  <motion.div
                    ...
                    initial={{
                      ...
                      left: leftOffset + 250, // Start off-screen right
                      top: topOffset,
                      scale: scale * 0.9, // Start slightly smaller
                      opacity: 0,
                      zIndex: zIndex, // Ensure zIndex is set correctly on initial
                    }}
                    animate={{
                      // Standard animate to the new, staggered position
                      left: leftOffset,
                      top: topOffset,
                      scale,
                      opacity: 1,
                    ...
                    exit={
                      isActive
                        ? {
                            // Animation for the card leaving the front position
                            x: 100, // Move quickly to the far right off-screen
                            y: 0,
                            scale: 0.8, // Shrink as it goes to the back
                            opacity: [1, 0], // Fade out at the end of the movement
                            zIndex: 0, // Drop to the lowest zIndex as it exits the stack
                            transition: {
                              duration: 0.6,
                              ease: "easeInOut",
                            },
                          }
                        : {} // Other visible cards don't explicitly exit, they just animate
                    }
                    transition={{
                      // Smooth spring for movement between staggered positions
                      layout: { type: "spring", stiffness: 150, damping: 22 },
                      default: { duration: 0.5, ease: "easeOut" },
                    }}
                    onClick={() => {
                      // Allow clicking a card to bring it to the front
                      if (index > 0) setCurrent(card.originalIndex);
                    }}
                  >
                    <motion.div
            ...
            <div className="relative h-[500px]">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {/* Mobile overlay content */}
                  <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black via-black/80 to-transparent p-6 rounded-b-2xl">
                    <h2 className="text-lg mb-3 text-white">
                      {cards[current].title}.
                    </h2>
                    <p className="text-gray-300 text-xs leading-relaxed mb-4 line-clamp-3">
